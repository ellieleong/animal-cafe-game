...
void draw() {
  ...
  for (FoodCircle fc : circles) {
    if (fc.beingDragged) {
      fc.update(); // Move with mouse
    }
    fc.display();
  }

  // Handle when a food item is being dragged
  if (draggingCircle != null && customer.thoughtBubbleVisible) {
    draggingCircle.display();

    // If food is dropped onto customer's request and matches their order
    if (customer.isPlaceholderHovered(draggingCircle.x, draggingCircle.y)) {
      if (customer.state.equals("waiting") && customer.orderType.equals(draggingCircle.type)) {
        money += foodCost; // Earn money for correct order
  ...
  // If the customer exits the screen, spawn a new one
